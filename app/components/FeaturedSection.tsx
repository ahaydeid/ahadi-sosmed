  ...
  return (
    <section className="px-4">
      ...
      <div className="flex gap-4 overflow-x-auto md:pb-4 pb-1 scrollbar-hide">
        {posts.map((post) => {
          // Determine the main image for the header (only from the reposter's content)
          // We DO NOT want to show the original post's image as the card's main image if it's a repost
          const image = post.imageUrl || extractFirstImage(post.description);
          const originalImage = post.repost_of?.imageUrl || (post.repost_of?.description ? extractFirstImage(post.repost_of.description) : null);
          ...
          return (
            ...
            >
              <div className="bg-white rounded-xs overflow-hidden hover:shadow-sm transition-shadow h-full flex flex-col border border-gray-100/50">
                {/* Image Header (Only for Reposter's own image) */}
                {image && (
                ...
                <div className="p-4 flex-1 flex flex-col">
                  ...
                  {/* Main content (Reposter's commentary) */}
                  {showTitle && (
                  ...
                  {/* REPOST QUOTE BOX (Original Post) */}
                  {isRepost && post.repost_of && (
                    <div className="mb-3 pl-3 border-l-3 border-gray-800 py-1 bg-gray-50/30 rounded-r-xs">
                      ...
                      {/* 3. Original Image (Big, Below Title) */}
                      {originalImage && (
